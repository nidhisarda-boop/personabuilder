...
def _rule_based_publishers(industry: str, signals: list) -> list:
    ...
    industry_map = {
        ...
        "finance":    [{"name":"eFinancialCareers","tier":"niche","why":"The definitive finance and accounting job board globally — 1M+ registered finance professionals."},
                       {"name":"AccountingJobs.com","tier":"niche","why":"Specialist board for CPA, CMA, audit, and accounting roles."},
                       {"name":"LinkedIn","tier":"premium","why":"Recruiters and headhunters dominate finance hiring — LinkedIn is the channel."}],
        "marketing":  [{"name":"Marketing Hire","tier":"niche","why":"Specialist board for marketing, brand, and growth roles — high context-match for marketing candidates."},
                       {"name":"LinkedIn","tier":"premium","why":"Primary discovery channel for marketing professionals — InMail and sponsored jobs both perform well."},
                       {"name":"HubSpot Job Board","tier":"niche","why":"Inbound marketing community board — strong for demand gen, content, and growth roles."},
                       {"name":"Built In","tier":"niche","why":"Strong for marketing roles at tech-adjacent companies — used by candidates who want modern tooling."}],
        "sales":      [{"name":"Sales Gravy","tier":"niche","why":"The leading sales-specific job board and community — used actively by quota-carrying AEs and BDRs."},
                       {"name":"LinkedIn","tier":"premium","why":"Sales professionals check LinkedIn actively for outreach and opportunities — sponsored jobs perform well."},
                       {"name":"Rainmakers","tier":"niche","why":"Curated platform for B2B sales talent — AEs and enterprise reps with verified track records."}],
        "hr":         [{"name":"SHRM HR Jobs","tier":"niche","why":"The Society for Human Resource Management's job board — highest-intent HR and talent acquisition candidates."},
                       {"name":"HR.com","tier":"niche","why":"2M+ HR community members — strong for HR generalists, L&D, and people ops roles."},
                       {"name":"ERE Recruiting Intelligence","tier":"niche","why":"Specialist board for talent acquisition, recruiting ops, and HR strategy professionals."}],
        ...
        "campus":     [{"name":"Handshake","tier":"niche","why":"16M students at 1,400+ universities — the campus recruiting standard."},
                       ...
                       {"name":"Internshala","tier":"niche","why":"Dominant internship and trainee board in South Asia — essential for India-based trainee/fresher roles."},
                       {"name":"Naukri Campus","tier":"niche","why":"India's largest campus hiring platform — critical for management trainee and fresher hiring in India."}],
    }
...
INDUSTRY_RE = {
    ...
    "finance":     r"\bfinance\b|fintech|\bbanking\b|\btrading\b|\baccounting\b|\baccountant\b"
                   r"|\bcpa\b|\bcma\b|\bcpa\b|\bacca\b|\baudit\b|wealth management|investment"
                   r"|\bfinancial\b|\bfinancials\b|financial analyst|financial record|actuar"
                   r"|accounts\s+payable|accounts\s+receivable|\bpayroll\b|\btaxation\b|\bbudget\b",
...
SENIORITY_RE = {
    ...
    # \btrainee\b catches "Management Trainee", "Graduate Trainee", "Sales Trainee" etc.
    # \bfresher\b catches common South Asian usage for fresh-graduate roles
    # fresh\s+grad(?:uate)? catches "fresh graduate" / "fresh grad" phrasing
...
SKILL_VOCAB_RE = [
    ...
    ("Tally ERP",        r"\bTally\s+(?:ERP|Prime|\.ERP)?\b"),
    ("SAP",              r"\bSAP\b(?!\s*(?:R&D|Labs))"),  # SAP ERP, not SAP Labs
    ("Oracle Financials", r"\bOracle\s+(?:Financials|EBS|E-Business\s+Suite|Fusion)\b"),
    ...
    ("Meta Ads",         r"\bMeta\s+Ads\b|\bFacebook\s+Ads\b"),
    ...
    ("Lever",            r"\bLever\b(?!\s+(?:arm|mechanism))"),
    ("BambooHR",         r"\bBambooHR\b"),
    # ── Aerospace / Mechanical engineering ───────────────────────────────────
    ("STOP Analysis",    r"\bSTOP\s+analysis\b|\bSTOP\s+model\b"),
    ("SolidWorks",       r"\bSolidWorks\b"),
    ("ANSYS",            r"\bANSYS\b"),
    ("Zemax",            r"\bZemax\b|\bOpticStudio\b"),
    ("CATIA",            r"\bCATIA\b"),
    ("NX CAD",           r"\bNX\s+CAD\b|\bSiemens\s+NX\b|\bUnigraphics\b"),
]
...
def _extract_salary(text: str) -> str:
    ...
    m = re.search(
        # Range with currency on both sides: $229,900–$262,400 or ₹35,000–₹50,000 or USD 137,300 – 185,700
        rf"{_CURR}[\d,]+\.?\d*[k]?\s*[-–—]\s*(?:{_CURR})?[\d,]+\.?\d*[k]?(?:\s*/\s*(?:hr|hour|yr|year|annually|month|mo)|\s+(?:annually|per\s+year|per\s+month|per\s+hour))?"
        # Single currency + per-unit: $35.50/hr or ₹56,000/month or ₹56,000 per month
        rf"|{_CURR}[\d,]+\.?\d*[k]?\s*(?:/\s*(?:hr|hour|yr|year|annually|month|mo)|per\s+(?:hr|hour|year|annum|month))"
        # Plain currency amount with k: $85k+ or ₹30k
        rf"|{_CURR}[\d,]+\.?\d*[k]\+?"
        # "From ₹56,000" / "From USD 50,000" patterns
        rf"|(?:from|starting\s+at|upto|up\s+to)\s+{_CURR}[\d,]+\.?\d*[k]?"
        # Numeric range without currency (requires nearby salary/compensation context)
...
def _detect_arrangement(text: str) -> str:
    ...
    if re.search(r"\bhybrid\b", text, re.IGNORECASE):
        return "Hybrid"
    if re.search(r"\bon.?site\b|\bin.?office\b|\bin\s+person\b|\bon\s+location\b|work\s+location\s*:\s*in\s+person", text, re.IGNORECASE):
        return "On-site"
...
def _detect_flags(text: str) -> dict:
    return {
        ...
        "veteran":    bool(re.search(
            ...
            text, re.IGNORECASE)),
        # campus: flag campus/intern/new-grad/trainee/fresher roles
...
6. DOMAIN DISCIPLINE — CRITICAL: NEVER use software-engineering terminology (tech debt, pull requests, PRs, code review, architecture review, engineering blog, test coverage, code quality, CI/CD, agile sprint) UNLESS the JD is explicitly for a software engineer, developer, or programmer. For marketing, finance, accounting, HR, operations, sales, legal, healthcare, logistics, or any non-software role, use ONLY domain-appropriate language from the JD itself. A marketing trainee persona should reference campaigns, growth, customer relationships — NOT code reviews or tech stacks. An accounting persona should reference compliance, reconciliation, ERP tools — NOT architecture reviews. Violating this rule makes the persona useless for the hiring team.
  ...
  "messaging_variants": [
    {
      ...
      "body": "2-3 sentences referencing THIS JD's domain tools, company type, scope. Use only terminology found in the JD. No platitudes.",
    ...
    },
    {
      ...
      "body": "2-3 sentences. Reference real communities or career moments relevant to THIS specific discipline (marketing, finance, engineering, etc.).",
      "cta": "Inviting CTA, max 8 words"
    },
    {
      "label": "Evidence/process (C-type)",
      "headline": "Max 12 words. Data-led, specific to this role's craft.",
      "body": "2-3 sentences. Include specifics from the JD — domain tools, scale, process, quality standards. No vague claims. Do NOT use software-engineering terms unless this is a software role.",
...
_MECE_ARCHETYPES: dict = {
    ...
    ("mid", "individual_contributor"): [
        {
            ...
            "disc": "C",
            "motivation": "Quality and precision — does things right, not just fast; has strong opinions on standards and how work is done.",
            ...
            "trigger": "Current team moves fast but accumulates process shortcuts or quality compromises that erode their standards.",
            "messaging_angle": "Lead with the domain tools named in the JD, quality expectations, and what 'done well' looks like on your team. Reference specific processes or tools from the JD — never invent engineering context for non-engineering roles.",
        },
        {
        ...
        },
        {
            ...
            "background_style": "Active in their professional community, industry Slack groups, or domain forums; values colleagues they can learn from and be energized by; may be selective based on team vibes.",
            "trigger": "Current job is technically fine but isolated — no team culture, no external presence, and no sense of shared mission.",
            "messaging_angle": "Lead with team events, community involvement, and what makes this team's culture distinctive. Reference domain-specific communities from the JD (e.g. marketing meetups, finance associations, engineering groups). Show the team as a community, not just a headcount.",
        },
    ],
    ...
    ("junior", "individual_contributor"): [
        {
            "archetype_name": "The Eager Starter",
            "disc": "D",
            "motivation": "Hands-on reps — wants real responsibilities, real exposure, and to own something meaningful early in their career.",
            "background_style": "Relevant educational background (degree, vocational training, or equivalent); eager proof-of-ability mindset; reads about the field obsessively; may have done internships or training programs.",
            "trigger": "Graduating or finishing a program; wants a full-time home where they can move fast and have real ownership from day one.",
            "messaging_angle": "Lead with early ownership, what they'll own or contribute in the first 90 days, and the caliber of teammates they'll learn from. Name real responsibilities from the JD. Show the ramp-up plan. Use domain-appropriate language, not engineering jargon.",
        },
        {
            "archetype_name": "The Thoughtful Learner",
            "disc": "S",
            "motivation": "Mentorship and safety — wants structure, a supportive review culture, and to learn from seniors without burning out.",
            "background_style": "Deliberate academic or training background; has mapped out what they want to build over 5 years; risk-averse about their first professional role.",
            "trigger": "Evaluating first or second role carefully — won't accept chaos or sink-or-swim environments.",
            "messaging_angle": "Lead with mentorship programs, senior investment in juniors, feedback culture, and psychological safety. Show what onboarding looks like. Avoid technical jargon for non-tech roles — use the language of the actual domain.",
        },
        {
            "archetype_name": "The Detail-Oriented Starter",
            "disc": "C",
            "motivation": "Getting it right — perfectionist streak; wants to understand the work deeply before acting; cares about standards and quality from day one.",
            "background_style": "Strong academic background with a focus on fundamentals; has studied the domain carefully and built disciplined working habits before entering the workforce.",
            "trigger": "Scared of joining a team that cuts corners — wants to start their career with good habits and strong examples.",
            "messaging_angle": "Lead with quality standards, documentation culture, and how seniors invest in junior development. Reference specific tools, processes, or standards from the JD. Show the team values doing things right, not just fast.",
        },
        {
            "archetype_name": "The Social Learner",
            "disc": "I",
            "motivation": "Community and growth — learns best by asking questions and being part of an engaged team; wants colleagues as much as a job.",
            "background_style": "Active in student organizations, industry communities, or professional networks; energized by group learning and peer feedback; thrives with open communication norms.",
            "trigger": "Wants to avoid a remote, quiet, or isolating first role — needs an environment where asking questions is welcomed and celebrated.",
            "messaging_angle": "Lead with team culture, social onboarding, and how approachable senior team members are. Show this is a team that invests in its junior members. Use domain-specific community references from the JD.",
        },
        {
            "archetype_name": "The Ambitious Achiever",
            "disc": "DC",
            "motivation": "Fast career growth with high standards — wants real responsibilities quickly but refuses to cut corners; has a chip on their shoulder about proving themselves.",
            "background_style": "Competitive academic or training background; has something to prove; has already completed a training program, internship, or side project they're proud of.",
            "trigger": "Doesn't want a purely administrative or low-ownership role early in their career — wants early exposure to interesting problems with high standards.",
            "messaging_angle": "Lead with what they'll contribute in their first 6 months, the quality bar expected, and how quickly strong juniors advance here. Tie career progression to JD-specific domain milestones.",
        },
        {
            ...
            "background_style": "First-generation professional or career changer entering a new domain; aware of gaps in their network and experience; looking for sponsorship as much as opportunity.",
            "trigger": "Evaluating whether the team will invest in them personally — not just assign them tasks and leave them to figure it out.",
            "messaging_angle": "Lead with formal mentorship programs, buddy systems, structured 30/60/90 plans, and examples of juniors who were sponsored into meaningful work. Use domain-specific language from the JD.",
        },
    ],
}
...
def _build_llm_prompt(signals: dict) -> str:
    ...
    if signals.get("mece_archetype"):
        ...
        parts.append(
            ...
            f"DOMAIN MAPPING: If the archetype definition uses engineering-specific language (e.g. 'tech stack', "
            f"'code quality', 'CS background', 'code review', 'bootcamp') but the JD is NOT for a software role, "
            f"translate that language into the actual domain of the JD. "
            f"'Tech stack' → the tools/software named in the JD. "
            f"'Code quality' → the quality/accuracy/compliance standard of this domain. "
            f"'CS background' → relevant educational background for this role. "
            f"Always use terminology from the JD, never invent engineering context for non-engineering roles."
        )
